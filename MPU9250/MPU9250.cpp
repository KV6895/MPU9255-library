...
bool MPU9250::setI2CBypassEnabled(const bool enabled) {
	uint8_t test = 0xff;
	if (enabled){
		return writeRegister(MPU9250_INT_PIN_CFG,MPU9250_BYPASS_EN_MASK);
	}
	else{
		return 0;
	}
...
bool MPU9250::gyroXIsEnabled(void) {
    //WARNING: this should be verified here
    uint8_t status = readMaskedRegister(MPU9250_PWR_MGMT_2, MPU9250_DISABLE_XG_MASK);
    return (status == 0);
}

bool MPU9250::gyroYIsEnabled(void) {
    //WARNING: this should be verified here
    uint8_t status = readMaskedRegister(MPU9250_PWR_MGMT_2, MPU9250_DISABLE_YG_MASK);
    return (status == 0);
}
...
void MPU9250::getMotion9(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz, int16_t* mx, int16_t* my, int16_t* mz) {
	...
	//writeRegister(MPU9250_PWR_MGMT_1,0x00);// initialize Power Manager Resister to 0
   
	//writeRegister(MPU9250_USER_CTRL,0x00); // Disable master I2C, enable value is 0x20, so disable it should be just bit 5 = 0, since I don't use other bits and they are all 0.
	
    //read magnetometer
    //setI2CBypassEnabled(true); //set i2c bypass enable pin to true to access magnetometer
	//writeRegister(MPU9250_MAG_CNTL,0x12); //writeMagRegister(MPU9250_MAG_CNTL, 0x01); //enable the magnetometer //continuous measurement mode1
